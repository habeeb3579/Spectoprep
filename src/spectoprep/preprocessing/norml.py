...
class Normalization(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if self.method == "minmax":
            ...
            # Guard against constant spectra (max == min).
            span = np.where(span == 0, 1.0, span)
            return ((X - min_vals) / span) * (range_max - range_min) + range_min
        elif self.method == "zscore":
...
class RowStandardizer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        # Apply row-wise standardization, guarding constant rows (std == 0).
        def _standardize(row):
            std = np.std(row)
            return (row - np.mean(row)) / (std if std != 0 else 1.0)

        return np.apply_along_axis(_standardize, axis=1, arr=X)
