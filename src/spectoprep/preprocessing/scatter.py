...
class StandardNormalVariate(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # Guard against constant spectra (std == 0) which would yield inf/nan.
...
class MultiplicativeScatterCorrection(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for spectrum in X:
            ...
            # Guard against a flat reference / zero slope (coef[0] == 0).
            slope = coef[0] if coef[0] != 0 else 1.0
            corrected = (spectrum - coef[1]) / slope
    ...
    def transform(self, X):
        ...
        def localized_snv(spectrum):
            ...
            for i in range(len(spectrum)):
                ...
                std = np.std(window)
                corrected[i] = (spectrum[i] - np.mean(window)) / (std if std != 0 else 1.0)
            return corrected
...
class RobustNormalVariate(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # Guard against a zero inter-percentile range (e.g. constant spectra).
